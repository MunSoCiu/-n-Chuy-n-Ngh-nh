<!DOCTYPE html>
<html lang="vi">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Mã giảm giá - Light Novel Hub</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
</head>
<body>
    <?php include '../../../includes/navbar.php'; ?>

    <div class="container py-5">
        <?php if (!empty($data['success'])): ?>
            <div class="alert alert-success"><?= $data['success'] ?></div>
        <?php endif; ?>

        <h2 class="mb-4">
            <i class="fas fa-tags"></i> Mã giảm giá của bạn
        </h2>

        <?php if ($data['promotions']->num_rows > 0): ?>
            <div class="row g-4">
                <?php while ($promo = $data['promotions']->fetch_assoc()): ?>
                    <div class="col-md-6">
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <h5 class="card-title mb-0">
                                        <i class="fas fa-percent text-danger"></i> 
                                        Giảm <?= $promo['discount_percentage'] ?>% cho tất cả sản phẩm
                                    </h5>
                                    <?php if ($data['is_admin'] || (strtotime($promo['end_date']) >= time())): ?>
                                        <span class="badge bg-success">Còn hiệu lực</span>
                                    <?php else: ?>
                                        <span class="badge bg-danger">Hết hạn</span>
                                    <?php endif; ?>
                                </div>
                                <div class="d-flex align-items-center mb-3">
                                    <div class="border rounded p-2 flex-grow-1 me-2 bg-light">
                                        <code class="fs-5"><?= $promo['code'] ?></code>
                                    </div>
                                    <button class="btn btn-primary" onclick="copyCode('<?= $promo['code'] ?>')">
                                        <i class="fas fa-copy"></i> Sao chép
                                    </button>
                                </div>
                                <p class="card-text text-muted mb-0">
                                    <i class="fas fa-clock me-1"></i>
                                    Hết hạn: <?= date('d/m/Y H:i', strtotime($promo['end_date'])) ?>
                                </p>
                                
                                <?php if ($data['is_admin']): ?>
                                    <div class="mt-3">
                                        <form method="POST" class="d-flex gap-2">
                                            <input type="hidden" name="action" value="assign">
                                            <input type="hidden" name="promo_id" value="<?= $promo['promo_id'] ?>">
                                            <input type="number" name="user_id" class="form-control" placeholder="User ID" required>
                                            <button type="submit" class="btn btn-sm btn-outline-primary">
                                                Gán cho user
                                            </button>
                                        </form>
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                <?php endwhile; ?>
            </div>
        <?php else: ?>
            <div class="alert alert-info">
                <i class="fas fa-info-circle me-2"></i>
                Hiện tại không có mã giảm giá nào khả dụng.
            </div>
        <?php endif; ?>

        <?php if (!$data['is_admin'] && $data['user_promotions'] && $data['user_promotions']->num_rows > 0): ?>
            <h3 class="mt-5 mb-4">Mã giảm giá của bạn</h3>
            <div class="row g-4">
                <?php while ($user_promo = $data['user_promotions']->fetch_assoc()): ?>
                    <div class="col-md-6">
                        <div class="card border-success">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <h5 class="card-title mb-0 text-success">
                                        <i class="fas fa-gift"></i> 
                                        Giảm <?= $user_promo['discount_percentage'] ?>%
                                    </h5>
                                    <span class="badge bg-success">Riêng cho bạn</span>
                                </div>
                                <div class="d-flex align-items-center mb-3">
                                    <div class="border border-success rounded p-2 flex-grow-1 me-2 bg-light">
                                        <code class="fs-5 text-success"><?= $user_promo['code'] ?></code>
                                    </div>
                                    <button class="btn btn-success" onclick="copyCode('<?= $user_promo['code'] ?>')">
                                        <i class="fas fa-copy"></i> Sao chép
                                    </button>
                                </div>
                                <p class="card-text text-muted mb-0">
                                    <i class="fas fa-clock me-1"></i>
                                    Hết hạn: <?= date('d/m/Y H:i', strtotime($user_promo['end_date'])) ?>
                                </p>
                            </div>
                        </div>
                    </div>
                <?php endwhile; ?>
            </div>
        <?php endif; ?>
    </div>

    <?php include '../../../includes/footer.php'; ?>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <script>
    function copyCode(code) {
        navigator.clipboard.writeText(code).then(() => {
            alert('Đã sao chép mã giảm giá: ' + code);
        });
    }
    </script>
</body>
</html>
